    ...
    try {
        ...
        if (db.profil) {
            ...
            if (db.profil.logo) {
                ...
                const loginPrev = document.getElementById('login-logo-preview');
                if (loginPrev) loginPrev.src = db.profil.logo;
            }
...
document.querySelectorAll('.sidebar-link').forEach(link => {
    link.addEventListener('click', function() {
        ...
        document.getElementById('header-title').innerText = this.innerText;
...
window.closeScannerModal = function() { overlay.classList.add('hidden'); stopScanner(); };
...
document.getElementById('form-jadwal').addEventListener('submit', function(e) {
    ...
    processImage(qrInput.files[0], async function(base64) {
        ...
        if (res.ok) {
            ...
            alert('❌ Gagal menyimpan event ke server');
        }
    });
});
